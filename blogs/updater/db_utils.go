...
func getPost(tx *sqlx.Tx, category database.Category, internalID string) (database.Post, error) {
	_, err := tx.Exec(
		"INSERT INTO post (category_id, internal_id, title, summary, date) VALUES ($1, $2, $3, $4, $5)",
...
func getTags(post database.Post) ([]database.Tag, error) {
	...
	err := database.DB.Select(&tags, "SELECT tag.* FROM tag JOIN post_to_tag ON post_to_tag.tag_id = tag.id JOIN post ON post.id = post_to_tag.post_id WHERE post.id=$1", post.ID)
	return tags, err
}
